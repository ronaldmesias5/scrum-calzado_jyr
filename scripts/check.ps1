...
Get-ChildItem "$versionsDir/*.py" | ForEach-Object {
    ...
    if ($content -match 'revision\s*=\s*"([^"]+)"') {
        ...
        if ($revId.Length -gt 32) {
            $longIds += "$($_.Name): revision='$revId' ($($revId.Length) chars)"
        }
    }
}
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "  FAIL — ruff check encontró errores:" -ForegroundColor Red
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "  FAIL — pytest falló:" -ForegroundColor Red
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "  FAIL — tsc -b encontró errores:" -ForegroundColor Red
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "  FAIL — pnpm test falló:" -ForegroundColor Red
...
Write-Host "========================================" -ForegroundColor Cyan
if ($failed -gt 0) {
    Write-Host "  $failed check(s) FALLARON — NO hacer push" -ForegroundColor Red

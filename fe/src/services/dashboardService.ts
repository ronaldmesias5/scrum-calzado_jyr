...
export const getMetrics = async (): Promise<Metric[]> => {
  try {
    const [pendiente, en_progreso, completado, usuarios] =
      await Promise.allSettled([
        api.get('/api/v1/admin/orders', {
          params: { page: 1, page_size: 1, state: 'pendiente' }
        }),
        api.get('/api/v1/admin/orders', {
          params: { page: 1, page_size: 1, state: 'en_progreso' }
        }),
        api.get('/api/v1/admin/orders', {
          params: { page: 1, page_size: 1, state: 'completado' }
        }),
        api.get('/api/v1/admin/users/pending-validation')
      ]);

    const val = (
      r: PromiseSettledResult<{ data: { total?: number; length?: number } }>,
      fallback = 0
    ): number => {
      if (r.status !== 'fulfilled') return fallback;
      return (
        r.value.data?.total ??
        (Array.isArray(r.value.data) ? r.value.data.length : fallback)
      );
    };
    ...
    return [
      {
        ...
        value: val(
          pendiente as PromiseSettledResult<{ data: { total?: number } }>
        )
      },
      {
        label: 'En Producción',
        value: val(
          en_progreso as PromiseSettledResult<{ data: { total?: number } }>
        )
      },
      {
        label: 'Pedidos Completados',
        value: val(
          completado as PromiseSettledResult<{ data: { total?: number } }>
        )
      },
      {
        label: 'Usuarios por Validar',
        value: val(
          usuarios as PromiseSettledResult<{ data: { total?: number } }>
        )
      }
    ];
  } catch {

...
export async function getLosses(params?: {
  ...
  const res = await api.get<LossRecordListResponse>(`${SCRAP_PREFIX}/losses`, {
    params
  });
  return res.data;
}
...
export async function approveLoss(id: string): Promise<LossRecord> {
  const res = await api.patch<LossRecord>(
    `${SCRAP_PREFIX}/losses/${id}/approve`
  );
  return res.data;
}
...
export async function getPendingIncidences(
  statusFilter?: string
): Promise<PendingIncidenceListResponse> {
  const params = statusFilter ? { status_filter: statusFilter } : {};
  const res = await api.get<PendingIncidenceListResponse>(
    `${PENDING_PREFIX}/pending-incidences`,
    { params }
  );
  return res.data;
}
...
export async function approvePendingIncidence(
  ...
  const res = await api.post<PendingProductIncidence>(
    `${PENDING_PREFIX}/pending-incidences/${id}/approve`,
    {

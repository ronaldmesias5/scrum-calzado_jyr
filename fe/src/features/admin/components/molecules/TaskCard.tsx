...
const HEADER_COLORS: Record<string, string> = {
  corte:
    'bg-amber-600 dark:bg-amber-600 border-amber-700 dark:border-amber-700',
  guarnicion:
    'bg-indigo-600 dark:bg-indigo-500 border-indigo-700 dark:border-indigo-600',
...
export const TaskCard: React.FC<TaskCardProps> = ({
  ...
  const headerColor =
    HEADER_COLORS[task.type] ||
    'bg-gray-50/50 dark:bg-slate-900/50 border-gray-100 dark:border-slate-700';
  const borderColor =
    BORDER_COLORS[task.type] || 'border-gray-200 dark:border-slate-700';
  ...
  const isPendienteSinAsignar =
    task.status === 'pendiente' && !task.assigned_to;
  const isInProgress = task.status === 'en_progreso';
  const isPendienteConAsignado =
    task.status === 'pendiente' && !!task.assigned_to;
   ...
   */
  const showEmployeeAssign =
    isPendienteSinAsignar && !!onAssignEmployee && employees.length > 0;
  ...
  return (
    ...
    >
      {/* Header Card */}
      <div
        className={`${compact ? 'p-3' : 'p-4'} border-b ${headerColor} bg-gradient-to-br from-white/10 to-transparent`}
      >
        <div
          className={`flex items-center justify-between gap-3 ${compact ? 'mb-2' : 'mb-3'}`}
        >
          <div
            className={`px-2.5 py-1 rounded-md border flex items-center gap-1.5 text-[11px] font-black uppercase tracking-wider ${stageColor}`}
          >
            <StageIcon size={14} />
        ...
        {showProductInfo && (
          <div
            className={`flex items-center gap-3 ${compact ? 'mt-1' : 'mt-2'}`}
          >
            <div
              className={`${compact ? 'w-10 h-10' : 'w-12 h-12'} rounded-xl bg-white overflow-hidden border-2 border-white shadow-md shrink-0 flex items-center justify-center`}
            >
              {task.product_image ? (
                <img
                  src={
                    task.product_image.startsWith('http')
                      ? task.product_image
                      : `${API_URL}${task.product_image}`
                  }
        ...
        <div
          className={`flex items-center gap-2 ${compact ? 'mt-3 pt-2' : 'mt-4 pt-3'} border-t border-white/10`}
        >
          <div
            className={`${compact ? 'w-7 h-7' : 'w-8 h-8'} rounded-full bg-white/20 backdrop-blur-sm flex items-center justify-center shrink-0 border border-white/20`}
          >
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[9px] text-white font-bold uppercase tracking-widest drop-shadow-sm opacity-80">
              {showEmployeeAssign
                ...
                : task.assigned_user_occupation || 'Operario'}
            </p>
          </div>
        </div>
      </div>
      ...
      {/* Body Card */}
      <div
        className={`${compact ? 'p-3' : 'p-4'} flex flex-col ${compact ? 'gap-3' : 'gap-4'} flex-1`}
      >
        <div className="flex items-center justify-between gap-2 flex-wrap">
          ...
          <div
            className={`px-3 py-1 rounded-md text-[11px] font-black uppercase tracking-wider whitespace-nowrap shadow-sm ${
              isPaid
                ? 'bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-300 border border-green-200 dark:border-green-800'
                : isCompleted
                  ? 'bg-fuchsia-100 dark:bg-fuchsia-900/40 text-fuchsia-700 dark:text-fuchsia-300 border border-fuchsia-200 dark:border-fuchsia-800'
                  : isPendienteConAsignado || isInProgress
                    ? 'bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-300 border border-amber-200 dark:border-amber-800'
                    : isPendienteSinAsignar
        ...
        {pricePerDozen > 0 && (
          <div
            className={`${compact ? 'p-1.5' : 'p-2'} bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-100 dark:border-green-800/50`}
          >
            <div className="flex items-center justify-between gap-1.5 mb-0.5">
              <span className="text-[10px] font-black text-green-800 dark:text-green-300 uppercase tracking-tighter">
                Costo Laboral
              </span>
            ...
            <div className="flex items-baseline gap-1">
              <span
                className={`${compact ? 'text-[13px]' : 'text-sm'} font-black text-green-700 dark:text-green-400`}
              >
                {formatCOP(totalCost)}
              </span>
              <span className="text-[8px] font-bold text-green-600/70 dark:text-green-400/50">
                ({formatCOP(pricePerDozen)}/doc)
              </span>
            </div>
          </div>
        )}
        ...
        <div className="flex flex-col gap-3 mt-auto pt-2">
          {claimable && onClaim && isPendienteSinAsignar ? (
            ...
            onCompleteTask &&
            (isInProgress || isPendienteConAsignado) ? (
            ...
            <select
              value={task.status}
              onChange={(e) =>
                onUpdateStatus && onUpdateStatus(task.id, e.target.value)
              }
            ...
            <div
              className={`w-full text-[10px] font-black uppercase tracking-wider text-amber-700 dark:text-amber-300 bg-amber-50 dark:bg-amber-900/20 border-2 border-amber-200 dark:border-amber-700 rounded-xl ${compact ? 'px-3 py-2' : 'px-4 py-3'} text-center`}
            >
              🔒 Etapa anterior pendiente
            </div>
          ) : isPaid ? (
            <div
              className={`w-full text-[10px] font-black uppercase tracking-wider text-green-700 dark:text-green-300 bg-green-50 dark:bg-green-900/20 border-2 border-green-200 dark:border-green-700 rounded-xl ${compact ? 'px-3 py-2' : 'px-4 py-3'} text-center font-black`}
            >
              ✅ TAREA PAGADA
            </div>
          ) : isCompleted ? (
            <div
              className={`w-full text-[10px] font-black uppercase tracking-wider text-fuchsia-700 dark:text-fuchsia-300 bg-fuchsia-50 dark:bg-fuchsia-900/20 border-2 border-fuchsia-200 dark:border-fuchsia-700 rounded-xl ${compact ? 'px-3 py-2' : 'px-4 py-3'} text-center font-black`}
            >
              ⏳ POR LIQUIDAR
            </div>
          ) : (
            <div
              className={`w-full text-[10px] font-black uppercase tracking-wider text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-900/20 border-2 border-gray-200 dark:border-gray-700 rounded-xl ${compact ? 'px-3 py-2' : 'px-4 py-3'} text-center`}
            >
              ❌ Cancelada
            </div>
          )}
          ...
          {onViewOrder && (
            <button
              onClick={(e) => {
                e.stopPropagation();
                onViewOrder(task.order_id, task.product_id);
              }}

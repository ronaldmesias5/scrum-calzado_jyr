  ...
  return (
    ...
    >
      <div className="flex flex-col">
        {/* Header decoration */}
        <div
          className={`${isActivating ? 'bg-green-50 dark:bg-green-900/10' : 'bg-red-50 dark:bg-red-900/10'} px-6 py-2 mb-4 border-b ${isActivating ? 'border-green-100 dark:border-green-900/20' : 'border-red-100 dark:border-red-900/20'}`}
        >
          <p
            className={`text-[10px] font-black uppercase tracking-widest ${isActivating ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}
          >
        ...
        <div className="p-6 space-y-4 bg-white dark:bg-slate-900">
          <div className="bg-gray-50 dark:bg-slate-800/40 rounded-2xl p-4 border border-gray-100 dark:border-slate-800 transition-all">
            <div className="flex items-center gap-3 mb-4">
              <div
                className={`w-12 h-12 rounded-full flex items-center justify-center text-white font-black text-lg shadow-inner ${employee.is_active ? 'bg-blue-600' : 'bg-gray-400 dark:bg-slate-700'}`}
              >
                {employee.name[0]}
          ...
          {!isActivating && (
            <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-900/50 rounded-xl p-4 flex gap-3 transition-colors">
              ...
              <p className="text-sm text-amber-800 dark:text-amber-200 font-bold leading-snug">
                Esta acción restringirá el inicio de sesión para esta cuenta

...
export default function AdjustInventoryModal({
    ...
    try {
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : 'Error desconocido al guardar el inventario';
  ...
  const totalPairs = Object.values(sizeQuantities).reduce(
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="px-6 py-2 mb-2">
          <p className="text-sm font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">
            {product.name} • {product.brand_name} • {product.color}
          </p>
        </div>
            ...
            <>
              <div className="bg-blue-50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/30 rounded-2xl p-4 mb-8 transition-colors">
                <p className="text-sm text-blue-800 dark:text-blue-200 font-medium">
                  <span className="font-bold">
                    Ingresa la cantidad de pares disponibles para cada talla.
                  </span>{' '}
                  El total se actualizará automáticamente basándose en los datos
                  del sistema.
                </p>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-8 gap-3">
                  {shoeSize.map((size) => (
                    <div
                      key={size}
                      className="flex flex-col items-center gap-2 p-2 bg-gray-50 dark:bg-slate-800/40 rounded-xl border border-transparent hover:border-blue-200 dark:hover:border-blue-900/50 transition-all"
                    >
                      <label className="text-[10px] text-gray-500 dark:text-gray-400 font-black">
                        T. {size}
                      </label>
                      <input
                        type="number"
              ...
              <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="bg-green-50 dark:bg-green-900/10 border border-green-100 dark:border-green-900/30 rounded-2xl p-5 shadow-sm transition-all">
                  <p className="text-[10px] text-green-700 dark:text-green-400 font-black uppercase tracking-wider mb-1">
                    Total Pares Stock
                  </p>
                ...
                <div className="bg-gray-50 dark:bg-slate-800/60 border border-gray-100 dark:border-slate-700 rounded-2xl p-5 shadow-sm transition-all">
                  <p className="text-[10px] text-gray-600 dark:text-gray-400 font-black uppercase tracking-wider mb-1">
                    Tallas Activas
                  </p>
                  <p className="text-4xl font-black text-gray-600 dark:text-gray-300">
                    {
                      Object.values(sizeQuantities).filter(
                        (qty: number) => qty > 0
                      ).length
                    }
                    <span className="text-lg text-gray-400 dark:text-gray-500 ml-1">
                      /{shoeSize.length}
                    </span>
                  </p>
                </div>
              ...
              <div className="mt-8 bg-orange-50 dark:bg-orange-900/10 border border-orange-100 dark:border-orange-900/20 rounded-2xl p-6 transition-all">
                ...
                <p className="text-sm text-orange-800/80 dark:text-orange-300/80 mb-5 font-medium leading-relaxed">
                  Define el límite mínimo de stock global para este producto. Si
                  el total cae por debajo de este valor, el sistema marcará el
                  stock como{' '}
                  <span className="font-bold underlineDecoration-wavy">
                    Insuficiente
                  </span>
                  .
                </p>
                <div className="flex items-center gap-6">
                  ...
                  <div className="flex-1 p-4 bg-white/50 dark:bg-slate-900/50 rounded-2xl border border-orange-100 dark:border-orange-900/30">
                    ...
                    <span
                      className={`text-lg font-black flex items-center gap-2 ${insufficientThreshold > totalPairs ? 'text-orange-600 dark:text-orange-400' : 'text-green-600 dark:text-green-400'}`}
                    >
                      <div
                        className={`w-3 h-3 rounded-full ${insufficientThreshold > totalPairs ? 'bg-orange-500' : 'bg-green-500'}`}
                      ></div>
                      {insufficientThreshold > totalPairs
                        ? 'INSUFICIENTE'
                        : 'SUFICIENTE'}
                    </span>
                  </div>
        ...
        <div className="flex items-center justify-between p-6 border-t border-gray-100 dark:border-slate-800 bg-gray-50/50 dark:bg-slate-800/20 flex-shrink-0 transition-colors">
          ...
          >
            {loading ? (
              ...
              `Guardar Cambios (${totalPairs} pares)`
            )}
          </button>
        </div>
      </div>

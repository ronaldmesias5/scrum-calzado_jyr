...
export default function SummarySizer({
  categoryName,
  initialItems,
  onChange
}: SummarySizerProps) {
  ...
  const sizeAmounts = availableSizes.reduce(
    (acc, size) => {
          ...
          {(() => {
            ...
            const getBtnClass = (id: string, colorIdx: number) => {
              ...
              return `px-2 py-1.5 rounded-lg text-[9px] uppercase transition-all shadow-sm ${safeColor} text-white ${
                active
                  ? `ring-2 ring-offset-1 ring-blue-500 dark:ring-blue-600 font-black scale-[1.05]`
                  : `font-bold opacity-80 hover:opacity-100`
              }`;
            };
                ...
                <>
                  ...
                  {[2, 3, 4, 5].map((num, i) => (
                    <button
                      key={num}
                      onClick={() =>
                        applyFixedX(num, damaRange, `fixed-${num}-dama`)
                      }
                ...
                <>
                  ...
                  {[2, 3, 4, 5].map((num, i) => (
                    <button
                      key={num}
                      onClick={() =>
                        applyFixedX(num, cabFullRange, `fixed-${num}-cab`)
                      }
            ...
            if (categoryName === 'Infantil') {
              ...
              return (
                <div className="flex flex-col gap-2 w-full">
                  <div className="flex flex-wrap gap-1.5">
                    {[2, 3, 4, 5, 6].map((num, i) => (
                      <button
                        key={`s-${num}`}
                        onClick={() =>
                          applyFixedX(num, infSmall, `fixed-${num}-inf-s`)
                        }
                    ...
                    {[2, 3, 4, 5, 6].map((num, i) => (
                      <button
                        key={`l-${num}`}
                        onClick={() =>
                          applyFixedX(num, infLarge, `fixed-${num}-inf-l`)
                        }
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    <button
                      onClick={() => applyFixedX(1, infFull, 'curve-1-inf')}
                      className={getBtnClass('curve-1-inf', 2)}
                    >
                      1xT (21-32)
                    </button>
                    <button
                      onClick={() => applyFixedX(2, infFull, 'curve-2-inf')}
                      className={getBtnClass('curve-2-inf', 3)}
                    >
                      2xT (21-32)
                    </button>
                    <button
                      onClick={() => applyFixedX(3, infFull, 'curve-3-inf')}
                      className={getBtnClass('curve-3-inf', 4)}
                    >
                      3xT (21-32)
                    </button>
                    <button
                      onClick={() => applyFixedX(4, infFull, 'curve-4-inf')}

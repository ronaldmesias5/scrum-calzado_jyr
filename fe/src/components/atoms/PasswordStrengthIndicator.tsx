...
export function PasswordStrengthIndicator({ password }: { password: string }) {
  ...
  const requirements = useMemo<Requirement[]>(
    () => [
      {
        id: 'length',
        label: t('auth.passwordReq.length'),
        met: password.length >= 8
      },
      {
        id: 'upper',
        label: t('auth.passwordReq.upper'),
        met: /[A-Z]/.test(password)
      },
      {
        id: 'lower',
        label: t('auth.passwordReq.lower'),
        met: /[a-z]/.test(password)
      },
      {
        id: 'number',
      ...
      {
        id: 'special',
        label: t('auth.passwordReq.special'),
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-1.5 gap-x-4">
        {requirements.map((req) => (
          <div key={req.id} className="flex items-center gap-2">
            ...
            <span
              className={`text-[11px] ${req.met ? 'text-gray-600 dark:text-gray-300' : 'text-gray-400 dark:text-gray-500'}`}
            >
              {req.label}
            </span>
          </div>

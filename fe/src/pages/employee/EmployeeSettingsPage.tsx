...
export default function EmployeeSettingsPage() {
  ...
  const [appearance, setAppearance] = useState<AppearanceSettings>(() =>
    loadPref('emp_cfg_appearance', { compactView: false, fontSize: 'base' })
  );
  ...
  function handleSave() {
    try {
      localStorage.setItem('emp_cfg_notif', JSON.stringify(notif));
      localStorage.setItem('emp_cfg_lang', JSON.stringify(lang));
      localStorage.setItem('emp_cfg_appearance', JSON.stringify(appearance));
      localStorage.setItem(
        'emp_cfg_sessionTimeout',
            ...
            <Card title="Preferencias de Notificación">
              <div className="space-y-6">
                <ToggleSetting
                  ...
                  desc="Notificar cuando te asignen una nueva tarea."
                  enabled={notif.taskAssigned}
                  onToggle={() =>
                    setNotif((p) => ({ ...p, taskAssigned: !p.taskAssigned }))
                  }
                />
                <ToggleSetting
                  icon={Clock}
                  title="Recordatorio de plazo"
                  desc="Recordatorio cuando una tarea esté próxima a vencer."
                  enabled={notif.deadlineReminder}
                  onToggle={() =>
                    setNotif((p) => ({
                      ...p,
                      deadlineReminder: !p.deadlineReminder
                    }))
                  }
                />
                <ToggleSetting
                  icon={BarChartIcon}
                  title="Nuevo reporte compartido"
                  desc="Notificar cuando el jefe comparta un reporte contigo."
                  enabled={notif.sharedReport}
                  onToggle={() =>
                    setNotif((p) => ({ ...p, sharedReport: !p.sharedReport }))
                  }
                />
              </div>
            </Card>
              ...
              <Card title="Sesión">
                <div className="space-y-6">
                  <div className="space-y-2">
                    ...
                    />
                    <p className="text-xs text-gray-400">
                      La sesión permanecerá activa hasta que cierres sesión.
                    </p>
                  </div>
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-red-600 dark:text-red-400 font-medium mt-1">
                          ...
                          eliminará tu cuenta y todos tus datos asociados.
                        </p>
                        <button
                          onClick={async () => {

...
export default function EmployeeDashboardPage() {
  ...
  useEffect(() => {
    Promise.all([getEmployeeMetrics(), getEmployeeTasks(), getSharedReports()])
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 stagger-reveal">
        {loading
          ? Array.from({ length: 4 }).map((_, i) => (
              <div
                key={i}
                className="bg-white dark:bg-slate-900/50 rounded-xl border border-gray-200 dark:border-slate-800 p-6 shadow-sm animate-pulse"
              >
          ...
          : metrics.map((metric, i) => {
              ...
              const valueColor =
                VALUE_COLORS[metric.icon] || DEFAULT_VALUE_COLOR;
                ...
                >
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      ...
                      <p
                        className={`text-4xl font-bold ${valueColor} transition-colors duration-500`}
                      >
                        {metric.value}
                      </p>
                    </div>
                    <div
                      className={`${iconCfg.bgColor} p-3 rounded-xl border border-transparent dark:border-white/5 transition-all shadow-sm`}
                    >
            ...
            <h2 className="text-lg font-extrabold text-gray-900 dark:text-white flex items-center gap-2">
              ...
              {sharedReports.some((r) => !r.is_read) && (
                <span className="text-xs font-bold bg-purple-600 text-white px-2 py-0.5 rounded-full">
                  {sharedReports.filter((r) => !r.is_read).length} nuevo
                  {sharedReports.filter((r) => !r.is_read).length > 1
                    ? 's'
                    : ''}
                </span>
              )}

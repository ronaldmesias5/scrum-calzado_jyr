...
import ContactClientModal from '@/features/admin/components/molecules/ContactClientModal';
...
import { StatusBadge } from '@/features/admin/components/atoms/StatusBadgeComponent';
import { resolveImageUrl } from '@/services/catalogService';
import {
  checkProductSupplies,
  type ProductSuppliesCheckResponse
...
const STAGES_LOGIC = [
  ...
  },
  {
    ...
    color: 'border-green-500 bg-green-50/50 dark:bg-green-900/10',
    icon: Sparkles
  }
];
...
function OrderDetailView({
  ...
  setProductionModal: React.Dispatch<
  ...
  const tableDetails = useMemo(() => {
    ...
    return filtered.sort(
      (a, b) => parseInt(a.size || '0') - parseInt(b.size || '0')
    );
  ...
  const handleUpdateTaskStatus = async (taskId: string, newStatus: string) => {
    try {
      ...
      const taskToUpdate = Array.isArray(currentTasks)
        ? currentTasks.find((t) => t.id === taskId)
        : null;
      ...
      if (newStatus === 'completado' && isEmlantillado) {
        try {
          ...
          if (productId) {
            ...
            if (res?.details) {
              const allCompleted = res.details.every(
                (d) => d.state === 'completado'
              );
        ...
        showToast(
          '🎉 Vale Terminado - Producto Completado - Listo para Entrega!'
        );
      } else {
  ...
  const handleIniciarEtapa = async (stageKey: string, stageLabel: string) => {
    ...
    if (nextIndex > 0) {
      ...
      const prevTask = Array.isArray(currentTasks)
        ? currentTasks.find((t) => t?.type === prevStage?.key)
        : null;
      if (!prevTask || prevTask.status !== 'completado') {
        showToast(
          `Completar ${prevStage?.label || 'la etapa anterior'} primero`,
          'error'
        );
        return;
      }
    ...
    try {
      ...
      const amountForTask =
        stageKey === 'corte'
          ? selectedOption === 'A'
            ? productionModal?.missingCount || 0
            : productionModal?.totalCount || 0
          : corteTask?.amount ||
            (selectedOption === 'A'
              ? productionModal?.missingCount || 0
              : productionModal?.totalCount || 0);

      const productDetailsForMetadata = order.details.filter(
        (d) => d.product_id === productId
      );
      ...
      if (stageKey === 'corte') {
        try {
          const productDetails = order.details.filter(
            (d) => d.product_id === productId
          );
          ...
          showToast(
            '⚠️ Tarea creada pero hubo error al descontar del inventario',
            'error'
          );
        }
      }
      ...
      if (stageKey === 'corte') {
        try {
          ...
          const lines = updatedOrder.details.filter(
            (d) =>
              d.product_id?.toLowerCase() ===
              productionModal?.productId?.toLowerCase()
          );
          const newTotalToProduce = lines.reduce(
            (acc, l) =>
              acc +
              Math.max(
                0,
                l.amount - Math.max(0, Math.floor(l.stock_available ?? 0))
              ),
            0
          );
      ...
      const updatedList = allTasks.filter(
        (t) => t.product_id?.toLowerCase() === productId.toLowerCase()
      );
    ...
    try {
      ...
      if (currentProductId) {
        ...
        setCurrentTasks(
          allTasks.filter(
            (t) =>
              t.product_id?.toLowerCase() === currentProductId.toLowerCase()
          )
        );
      }
  ...
  const groups = order.details.reduce<Record<string, typeof order.details>>(
    (acc, d) => {
  ...
  const completedProducts = Object.values(groups).filter((lines) =>
    lines.every((l) => l.state === 'completado' || l.state === 'entregado')
  ).length;
  const inProgressProducts = Object.values(groups).filter((lines) =>
    lines.some((l) => l.state === 'en_progreso')
  ).length;
        ...
        <div className="flex items-center gap-4 w-full sm:w-auto">
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              {completedProducts < productCount &&
                productCount - completedProducts > 0 &&
                order.state !== 'entregado' && (
                  <span className="text-[10px] font-black text-orange-500 uppercase">
                    Faltan {productCount - completedProducts} productos
                  </span>
                )}
            </div>
          </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white dark:bg-slate-900/50 border border-gray-200 dark:border-slate-800 rounded-xl p-6 transition-all duration-300 stagger-reveal">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
              <div>
                <p className="text-gray-500 dark:text-gray-400 text-[10px] uppercase font-bold mb-1 tracking-widest">
                  ID Pedido
                </p>
          ...
          {order.details.length === 0 ? (
            ...
            (() => {
              ...
              const groups = order.details.reduce<
                Record<string, typeof order.details>
              >((acc, d) => {
                const key = d.product_id;
                if (!acc[key]) acc[key] = [];
                acc[key].push(d);
              ...
              return (
                <div className="space-y-4">
                  ...
                  {Object.entries(groups).map(([productId, lines]) => {
                    ...
                    const productName =
                      first.product_name ??
                      `Producto #${productId.substring(0, 8)}`;
                    ...
                    const productImageUrl = first.image_url
                      ? resolveImageUrl(first.image_url)
                      : null;

                    const totalProductPairs = lines.reduce(
                      (s, l) => s + l.amount,
                      0
                    );
                    ...
                    const taskForProduct = Array.isArray(currentTasks)
                      ? currentTasks.find(
                          (t) =>
                            t.product_id?.toLowerCase() ===
                    ...
                    return (
                      <div
                        key={productId}
                        className={`bg-white dark:bg-slate-900/50 border rounded-xl overflow-hidden shadow-sm transition-all duration-300 stagger-reveal ${allAvailable ? 'border-green-200 dark:border-green-900/50' : 'border-gray-200 dark:border-slate-800'}`}
                      >
                        <div
                          className={`${productState === 'completado' ? 'bg-green-50/50 dark:bg-green-950/20 border-green-100 dark:border-green-900/30' : productState === 'en_progreso' ? 'bg-blue-50/50 dark:bg-blue-950/20 border-blue-100 dark:border-blue-900/30' : 'bg-gray-50 dark:bg-slate-800/50 border-gray-200 dark:border-slate-800'} border-b px-5 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4`}
                        >
                          <div className="flex items-center justify-between w-full sm:w-auto gap-4">
                            <div className="flex items-center gap-4 min-w-0">
                              ...
                              {productImageUrl &&
                              !failedImages.has(productImageUrl) ? (
                                <button
                                  onClick={() => {
                                    setViewingImage(productImageUrl);
                                ...
                                >
                                  <Package
                                    className={`w-10 h-10 ${allAvailable ? 'text-green-300 dark:text-green-700' : 'text-gray-300 dark:text-gray-700'}`}
                                  />
                                </div>
                              )}
                              <div className="min-w-0">
                                ...
                                <p className="text-sm text-gray-600 dark:text-gray-400 mt-1 transition-colors">
                                  {[brandName, styleName, categoryName]
                                    .filter(Boolean)
                                    .join(' · ')}
                                  ...
                                  <button
                                    onClick={() =>
                                      setProductionModal({
                                        productId,
                                        productName,
                                        missingCount: totalToProduce,
                                        totalCount: totalProductPairs
                                      })
                                    }
                            ...
                            {[...lines]
                              ...
                              .map((line) => {
                                ...
                                return (
                                  <div
                                    key={line.id}
                                    className={`flex flex-col gap-1 border rounded-lg px-3 py-2 ${bgColor} ${borderColor}`}
                                  >
                        ...
                        {productState === 'completado' ? (
                          <div className="px-5 py-4 bg-green-50/50 dark:bg-green-950/20 border-t border-green-100 dark:border-green-900/30 space-y-3">
                            <div className="flex items-center justify-between mb-3">
                              <h3 className="text-xs font-black text-green-700 dark:text-green-400 uppercase tracking-widest">
                                ...
                                return (
                                  <div className="px-5 py-3 bg-green-50/50 dark:bg-green-950/10 border-t border-green-100 dark:border-green-900/30 flex items-center gap-2">
                                    ...
                                    <p className="text-[10px] font-bold text-green-700 dark:text-green-400 uppercase tracking-wider">
                                      No se requiere producción: Todo disponible
                                      en bodega
                                    </p>
                                  </div>
                                );
                                ...
                                <div className="px-5 py-4 bg-blue-50/20 dark:bg-slate-800/10 border-t border-gray-100 dark:border-slate-800 space-y-3">
                                  <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-2">
                                      ...
                                      <p className="text-[9px] text-gray-400 dark:text-gray-500 font-medium">
                                        (Calculados según requerimiento por
                                        docena en catálogo)
                                      </p>
                                    </div>
                                    <span className="px-2 py-1 bg-blue-600 text-white text-[10px] font-black rounded-lg uppercase">
                                  ...
                                  {!productSuppliesData ? (
                                    <p className="text-[10px] font-bold text-gray-400 dark:text-gray-500 italic">
                                      Cargando métricas de materiales...
                                    </p>
                                  ) : productSuppliesData.supplies.length ===
                                    0 ? (
                                    <p className="text-[10px] font-bold text-orange-500 dark:text-orange-400 italic">
                                      Este producto no tiene insumos vinculados
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                      {productSuppliesData.supplies.map((s) => {
                                        const neededMissing =
                                          totalToProduce * s.quantity_required;
                                        const neededTotal =
                                          totalProductPairs *
                                          s.quantity_required;
                                        const hasStockMissing =
                                          s.stock_quantity >= neededMissing;
                                        const hasStockTotal =
                                          s.stock_quantity >= neededTotal;
                                        ...
                                        return (
                                          <div
                                            key={s.supply_id}
                                            className="flex flex-col sm:flex-row sm:items-center justify-between p-3 rounded-xl border bg-white dark:bg-slate-900 border-gray-100 dark:border-slate-800 shadow-sm transition-all hover:border-blue-200 dark:hover:border-blue-900/40"
                                          >
                                            <div className="mb-2 sm:mb-0">
                                              ...
                                              <p className="text-[8px] font-bold text-gray-400 uppercase tracking-tighter">
                                                {s.supply_category || 'General'}
                                              </p>
                                              <div className="flex items-center gap-2 mt-1">
                                                <span className="text-[9px] font-black text-gray-400">
                                                  Stock:
                                                </span>
                                                <span className="text-[10px] font-black text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/30 px-1.5 py-0.5 rounded">
                                            ...
                                            <div className="grid grid-cols-2 sm:flex sm:flex-row gap-3 sm:gap-6 border-t sm:border-t-0 sm:border-l border-gray-50 dark:border-slate-800 pt-2 sm:pt-0 sm:pl-6">
                                              <div className="flex flex-col">
                                                ...
                                                <div className="flex items-center gap-1.5">
                                                  <span
                                                    className={`text-[10px] font-black ${hasStockMissing ? 'text-gray-900 dark:text-white' : 'text-red-600'}`}
                                                  >
                                                    {fmt(neededMissing)}
                                                  </span>
                                                  {hasStockMissing ? (
                                              ...
                                              <div className="flex flex-col">
                                                ...
                                                <div className="flex items-center gap-1.5">
                                                  <span
                                                    className={`text-[10px] font-black ${hasStockTotal ? 'text-gray-900 dark:text-white' : 'text-red-600'}`}
                                                  >
                                                    {fmt(neededTotal)}
                                                  </span>
                                                  {hasStockTotal ? (
        ...
        {productionModal &&
          createPortal(
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/70 backdrop-blur-md animate-in fade-in duration-300">
              <div
                className={`bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-2xl overflow-hidden border border-gray-100 dark:border-slate-800 transition-all duration-500 flex flex-col ${productionStep === 1 ? 'max-w-xl w-full' : 'max-w-4xl w-full max-h-[90vh]'}`}
              >
                {/* Header Modal */}
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                          ...
                          <span className="text-red-600 text-[16px] font-black">
                            ...
                            {(() => {
                              if (
                                ...
                                (loadingTasks && currentTasks.length === 0)
                              )
                                return '⏳';
                      ...
                      <p className="text-xs text-gray-500 font-bold uppercase tracking-widest flex items-center gap-2">
                        ...
                        {(() => {
                          const check =
                            orderSupplies[productionModal?.productId];
                  ...
                  {productionStep === 1 ? (
                    ...
                    <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-300">
                      <div className="text-center space-y-2">
                        <h3 className="text-lg font-black text-gray-800 dark:text-gray-200 uppercase">
                          ¿Qué deseas fabricar hoy?
                        </h3>
                        <p className="text-sm text-gray-500 dark:text-gray-400">
                          Selecciona la cantidad de pares para iniciar el lote
                          de trabajo
                        </p>
                      </div>
                      ...
                      (productionModal?.totalCount ?? 0) ? (
                          ...
                          <div className="relative group p-6 rounded-[2rem] border-2 border-orange-500 bg-orange-50/30 ring-4 ring-orange-500/10 cursor-pointer">
                            <div className="flex flex-col items-center justify-center mb-4 text-center space-y-2">
                              <span className="text-lg font-black text-orange-600 uppercase">
                                ⚠️ PRODUCCIÓN COMPLETA REQUERIDA
                              </span>
                              <span className="text-xs font-bold text-gray-500">
                                No hay unidades disponibles en bodega para este
                                producto.
                              </span>
                            </div>
                            <div className="flex justify-center mb-4">
                              <span className="px-6 py-2 bg-orange-500 text-white text-sm font-black rounded-full shadow-lg shadow-orange-500/20">
                                {productionModal?.totalCount || 0} PARES TOTALES
                              </span>
                            </div>
                              ...
                              <table className="w-full text-[10px] font-bold">
                                  ...
                                  <tr className="text-gray-400 uppercase tracking-widest border-b border-gray-50 dark:border-slate-800">
                                    ...
                                    {[
                                      ...
                                      .map((d) => (
                                        <th
                                          key={d.size}
                                          className="p-2 text-center border-l border-gray-50 dark:border-slate-800"
                                        >
                                          {d.size}
                                        </th>
                                      ))}
                                ...
                                <tbody>
                                  <tr className="text-gray-700 dark:text-gray-300">
                                    <td className="p-2 uppercase tracking-tighter text-orange-600">
                                      Fabricar
                                    </td>
                                    {[
                                      ...order.details.filter(
                                      ...
                                      .map((d) => (
                                        <td
                                          key={d.size}
                                          className="p-2 text-center border-l border-gray-50 dark:border-slate-800 font-black"
                                        >
                                          {d.amount}
                                        </td>
                                      ))}
                        ...
                        <div className="grid grid-cols-1 gap-6">
                            ...
                            >
                              <div className="flex items-center justify-between mb-4">
                                <div className="flex items-center gap-3">
                                  <div
                                    className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all ${selectedOption === 'A' ? 'border-blue-500 bg-blue-500' : 'border-gray-300'}`}
                                  >
                                    {selectedOption === 'A' && (
                                ...
                                <span className="px-4 py-1.5 bg-blue-600 text-white text-xs font-black rounded-full shadow-lg shadow-blue-600/20">
                                  {productionModal?.missingCount || 0} PARES
                                </span>
                              </div>
                          ...
                          <div
                            className={`relative group p-6 rounded-[2rem] border-2 transition-all cursor-pointer ${selectedOption === 'B' ? 'border-orange-500 bg-orange-50/30 ring-4 ring-orange-500/10' : 'border-gray-100 dark:border-slate-800 hover:border-gray-300 dark:hover:border-slate-700'}`}
                            onClick={() => setSelectedOption('B')}
                          >
                            <div className="flex items-center justify-between mb-4">
                              <div className="flex items-center gap-3">
                                <div
                                  className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all ${selectedOption === 'B' ? 'border-orange-500 bg-orange-500' : 'border-gray-300'}`}
                                >
                                  {selectedOption === 'B' && (
                                    <div className="w-2 h-2 bg-white rounded-full" />
                                  )}
                              ...
                              <span className="px-4 py-1.5 bg-orange-500 text-white text-xs font-black rounded-full shadow-lg shadow-orange-500/20">
                                {productionModal?.totalCount || 0} PARES
                              </span>
                            </div>
                      ...
                      <div className="space-y-4">
                        <div className="flex items-center justify-between px-2">
                          ...
                          <span className="text-[10px] font-bold text-gray-400 italic">
                            Asigna un responsable por cada etapa
                          </span>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {STAGES_LOGIC.map((stage, idx) => {
                            ...
                            const prevTask =
                              prevStage && Array.isArray(currentTasks)
                                ? currentTasks.find(
                                    (t) => t.type === prevStage.key
                                  )
                                : null;
                            const isUnreachable =
                              prevStage &&
...
>>>>>>> 257779c (feat: task priority and mobile admin improvements)
                            ...
                            return (
                              <div
                                key={stage.key}
                                className={`p-6 rounded-[2rem] border-2 transition-all shadow-sm ${stage.color} ${isUnreachable ? 'opacity-50 grayscale pointer-events-none bg-gray-100 dark:bg-slate-800' : ''}`}
                              >
                                <div className="flex items-center justify-between mb-4">
                                  <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                      ...
                                      {(() => {
                                        ...
                                        const taskCost = Math.round(
                                          (pares / 12) * pricePerDozen
                                        );
                                        return (
                                          <div className="mt-1 flex flex-col">
                                            <p className="text-[9px] font-bold text-gray-500 dark:text-gray-400">
                                              {formatCOP(pricePerDozen)} /
                                              docena
                                            </p>
                                      ...
                                      <div className="flex flex-col gap-3 p-3 bg-white/60 dark:bg-slate-900/60 rounded-xl border border-black/5">
                                        <div className="flex items-center gap-3">
                                          ...
                                          <div className="flex-1 min-w-0">
                                            ...
                                            <p className="text-sm font-black text-gray-900 dark:text-white uppercase tracking-tight leading-none truncate">
                                              {currentTask?.assigned_user_name ||
                                                'Asignado'}
                                            </p>
                                          </div>
                                          <div
                                        ...
                                        <div className="mt-2 pt-2 border-t border-gray-100 dark:border-slate-800">
                                          {(() => {
                                            ...
                                            const nextIndex = currentIndex + 1;
                                            const nextStage =
                                              nextIndex < STAGES_LOGIC.length
                                                ? STAGES_LOGIC[nextIndex]
                                                : null;
                                            const nextTaskExists =
                                              nextStage &&
                                              Array.isArray(currentTasks) &&
                                              currentTasks.some(
                                  ...
                                  <div className="space-y-2">
                                    ...
                                    <div className="flex flex-wrap gap-1.5">
                                      {(() => {
                                        ...
                                        if (stageSupplies.length === 0)
                                          return (
                                            <span className="text-[10px] text-gray-400 italic">
                                              No hay insumos específicos
                                            </span>
                                          );
                                        ...
                                        return stageSupplies.map((is) => {
                                          ...
                                          return (
                                            <span
                                              key={is.supply_id}
                                              className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white dark:bg-slate-900/50 text-[10px] font-bold text-gray-700 dark:text-gray-300 rounded-lg border border-black/5 dark:border-white/5"
                                            >
                ...
                {productionStep === 1 && (
                  <div className="px-8 py-4 bg-gray-50 dark:bg-slate-800/20 border-t border-gray-100 dark:border-slate-800 flex items-center gap-3">
                    ...
                    <p className="text-[10px] text-gray-500 font-bold uppercase tracking-tighter">
                      Recuerda revisar el stock de insumos antes de confirmar la
...
export default function OrdersPage() {
  ...
  const [orderSupplies, setOrderSupplies] = useState<
    Record<string, ProductSuppliesCheckResponse>
  >({});
  ...
  useEffect(() => {
    if (autoOpened && selectedOrder) {
      ...
      if (productId && selectedOrder) {
        const item = selectedOrder.details.find(
          (d) => d.product_id === productId
        );
        if (item) {
          ...
          getOrderTasks(selectedOrder.id)
            .then((allTasks) => {
              const productsTasksForModal = allTasks.filter(
                (t) => t.product_id?.toLowerCase() === productId.toLowerCase()
              );
              const itemsForProduct = selectedOrder.details.filter(
                (d) => d.product_id === productId
              );
              const totalProductPairsForProduct = itemsForProduct.reduce(
                (sum, d) => sum + d.amount,
                0
              );
              ...
              if (productsTasksForModal.length > 0) {
                ...
                displayMissingCount =
                  productsTasksForModal[0]?.amount || displayMissingCount;
              }
  ...
  const handleSelectOrder = async (orderOrId: Order | string) => {
    ...
    try {
      ...
      const productIds = Array.from(
        new Set(detail.details.map((d) => d.product_id))
      );
  ...
  const handleCompleteFromWarehouse = async (
    ...
    try {
      ...
      setOrders((prev) =>
        prev.map((o) =>
          o.id === refreshed.id ? { ...o, state: refreshed.state } : o
        )
      );
  ...
  const handleUpdateItemsStatus = async (
    productId: string,
  ...
  const handleStatusChange = async (
    ...
    try {
      ...
      setOrders((prev) =>
        prev.map((o) => (o.id === orderId ? { ...o, state: newStatus } : o))
      );
          ...
          onSuccess={async () => {
            ...
            try {
              ...
              setOrders((prev) =>
                prev.map((o) =>
                  o.id === refreshed.id
                    ? { ...o, total_pairs: refreshed.total_pairs }
                    : o
                )
              );
              await calculateTotals();
        ...
        <div className="bg-white dark:bg-slate-900/50 border border-gray-200 dark:border-slate-800 rounded-xl p-20 text-center shadow-sm flex flex-col items-center gap-4 transition-all stagger-reveal">
          ...
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-1 transition-colors">
              Prueba ajustando los filtros de búsqueda o el rango de fechas
            </p>
          </div>
          <button
            onClick={() => {

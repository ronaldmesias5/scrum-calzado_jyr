...
const TruncatedCell = ({
  ...
  return (
    <span
      className="text-xs text-gray-700 dark:text-gray-300 block max-w-[200px] truncate"
      title={text}
    >
      {display}
    </span>
...
export default function LossesPage() {
  ...
  const [repairIncidentData, setRepairIncidentData] =
  ...
  const handleShareSubmit = async () => {
    ...
    if (!shareTarget.order_id && !shareEmail.trim()) {
      showToast(
        'Esta incidencia no tiene pedido vinculado: escribe un correo destino.',
        'error'
      );
      return;
    }
    ...
    try {
      ...
      const parts: string[] = [];
      if (resp.shared_internally)
        parts.push('visible en el dashboard del cliente');
      if (resp.email_sent) parts.push('correo enviado');
      showToast(
        `Incidencia compartida: ${parts.join(' · ') || resp.detail}`,
        'success'
      );
      setShareTarget(null);
    } catch (e) {
  ...
  const [pendingIncidences, setPendingIncidences] = useState<
  ...
  const totalQuantity = Math.round(
    incidents.reduce((sum, inc) => sum + (Number(inc.quantity) || 0), 0)
  );
  const totalPerdida = Math.round(
    incidents
      .filter((inc) => inc.incident_type === 'perdida')
      .reduce((sum, inc) => sum + (Number(inc.quantity) || 0), 0)
  );
  const totalReparado = Math.round(
    incidents
  ...
  const handleRepairConfirm = async () => {
    ...
    try {
      ...
      showToast(
        e.response?.data?.detail || 'Error al reparar incidencia',
        'error'
      );
    } finally {
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <p className="text-gray-500 dark:text-gray-400 max-w-md">
          Solo los administradores y jefes pueden acceder a la sección de
          incidencias.
        </p>
      </div>
    );
                    ...
                    {incidents.map((inc) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-center">
                          <span
                            className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${
                              CATEGORY_BADGE[inc.incidence_category] ??
                              'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400'
                            }`}
                          >
                            {CATEGORY_DISPLAY[inc.incidence_category] ??
                              inc.incidence_category}
                          </span>
                        </td>
                          ...
                          {inc.incidence_category === 'producto'
                            ...
                            : inc.incidence_category === 'maquinaria'
                              ...
                              : inc.incidence_category === 'insumo'
                                ? inc.custom_supply_name ||
                                  inc.supply?.name_supplies ||
                                  '—'
                                : (inc.product?.name_product ?? '—')}
                        </td>
                        ...
                        <td className="px-4 py-3 text-center">
                          <span
                            className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${
                              INCIDENT_TYPE_BADGE[inc.incident_type] ??
                              'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400'
                            }`}
                          >
                            {INCIDENT_TYPE_DISPLAY[inc.incident_type] ??
                              inc.incident_type}
                          </span>
                        </td>
                        ...
                        <td className="px-4 py-3 text-center">
                          {inc.order_id ? (
                            <button
                              ...
                              onClick={() =>
                                navigate(
                                  `/dashboard/admin/orders?order=${inc.order_id}`
                                )
                              }
                        ...
                        <td className="px-4 py-3 text-center">
                          ...
                          {(inc.incident_type === 'falla' ||
                            inc.incident_type === 'faltante') && (
                            <button
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-800 shadow-sm overflow-hidden">
          {repairedLoading ? (
            <div className="flex flex-col items-center justify-center py-20 gap-3">
              <div className="w-8 h-8 border-4 border-blue-600 border-t-transparent rounded-full animate-spin" />
              <p className="text-gray-500 dark:text-gray-400 font-medium">
                Cargando historial de reparados...
              </p>
            </div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-800 shadow-sm overflow-hidden">
          {scrapLoading ? (
            <div className="flex flex-col items-center justify-center py-20 gap-3">
              <div className="w-8 h-8 border-4 border-blue-600 border-t-transparent rounded-full animate-spin" />
              <p className="text-gray-500 dark:text-gray-400 font-medium">
                Cargando stock de recuperables...
              </p>
            </div>
          ) : scrapStock.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 gap-4 text-center px-4">
              ...
              <div>
                <p className="text-gray-900 dark:text-white font-bold text-lg">
                  No hay stock de recuperables
                </p>
                <p className="text-gray-500 dark:text-gray-400 mt-1">
                  Las incidencias registradas aparecerán aquí como stock de
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-slate-800">
                  {scrapStock.map((item, index) => (
                    <tr
                      key={
                        item.id ?? `${item.product_id}-${item.size}-${index}`
                      }
                      className="hover:bg-gray-50 dark:hover:bg-slate-800/40 transition-colors"
                    >
                      ...
                      <td className="px-4 py-3">
                        <TruncatedCell
                          text={item.description ?? item.defect_code?.code}
                        />
                      </td>
                    </tr>
                  ))}
          ...
          <div>
            {pendingLoading ? (
              <div className="flex flex-col items-center justify-center py-20 gap-3">
                <div className="w-8 h-8 border-4 border-blue-600 border-t-transparent rounded-full animate-spin" />
                <p className="text-gray-500 dark:text-gray-400 font-medium">
                  Cargando incidencias pendientes...
                </p>
              </div>
                        ...
                        <div className="border-t border-gray-200 dark:border-slate-700 pt-3 mt-3">
                          ...
                          {inc.approved_type && (
                            <p className="text-xs text-green-600 dark:text-green-400 mt-1">
                              Tipo:{' '}
                              {inc.approved_type === 'perdida'
                                ? 'Pérdida'
                                : inc.approved_type === 'en_reparacion'
                                  ? 'En Reparación'
                                  : 'Devuelto'}
                            </p>
                          )}
            ...
            >
              ...
              <div className="mb-4 p-3 bg-gray-50 dark:bg-slate-700 rounded-lg">
                ...
                <p className="font-medium text-gray-900 dark:text-gray-100">
                  {repairIncidentData.product?.name_product ||
                    repairIncidentData.product_id}
                </p>
                <div className="flex gap-4 mt-2">
                  <div>
                    <span className="text-xs text-gray-500">Talla:</span>{' '}
                    <span className="font-medium">
                      {repairIncidentData.size}
                    </span>
                  </div>
                  <div>
                    <span className="text-xs text-gray-500">Cantidad:</span>{' '}
                    <span className="font-medium">
                      {repairIncidentData.quantity}
                    </span>
                  </div>
                  <div>
                    <span className="text-xs text-gray-500">Tipo:</span>{' '}
                    <span className="font-medium">
                      {INCIDENT_TYPE_DISPLAY[repairIncidentData.incident_type]}
                    </span>
                  </div>
                </div>
              </div>

              {repairIncidentData.incident_type === 'en_reparacion' &&
              repairIncidentData.order_id ? (
                <div className="mb-4">
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                    Destino de la reparación:
                  </label>
                  <select
                  ...
                  >
                    ...
                    <option value="reserva">
                      Restaurar al pedido + Reserva (completado)
                    </option>
                  </select>
                </div>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl max-w-lg w-full p-6 animate-in fade-in zoom-in duration-200">
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400">
                      El cliente la verá en su dashboard y/o correo
                    </p>
                  </div>
                </div>
                <button
              ...
              <div className="mb-4 p-3 bg-gray-50 dark:bg-slate-700/50 rounded-xl">
                <p className="text-sm font-bold text-gray-900 dark:text-white">
                  {shareTarget.product?.name_product || 'Incidencia'}
                </p>
                <div className="flex gap-4 mt-1.5 text-xs text-gray-500 dark:text-gray-400">
                  ...
                  <span>
                    Cantidad:{' '}
                    <strong className="text-gray-800 dark:text-gray-200">
                      {Number(shareTarget.quantity)}
                    </strong>
                  </span>
                  <span>
                    Tipo:{' '}
                    <strong className="text-gray-800 dark:text-gray-200">
                      {INCIDENT_TYPE_DISPLAY[shareTarget.incident_type]}
                    </strong>
                  </span>
                  {shareTarget.order_id && (
                    <span>
                      ...
                      <strong className="text-blue-600 dark:text-blue-400 font-mono">
                        #{shareTarget.order_id.slice(0, 8)}
                      </strong>
                    </span>
                  )}
                </div>
              ...
              {shareTarget.order_id && (
                <div className="mb-4 flex items-start gap-2 rounded-xl border border-blue-100 dark:border-blue-900/40 bg-blue-50/70 dark:bg-blue-900/20 px-3 py-2.5">
                  ...
                  <p className="text-xs text-blue-800 dark:text-blue-300">
                    Se compartirá automáticamente con el cliente de este pedido
                    en su dashboard.
                  </p>
                </div>
              )}
              ...
              <label className="block mb-4">
                ...
                {!shareTarget.order_id && (
                  <span className="block mt-1 text-[11px] text-red-500">
                    Obligatorio: esta incidencia no tiene pedido vinculado.
                  </span>
                )}
              </label>
              ...
              <label className="block mb-5">
                ...
                <textarea
                  value={shareMessage}
                  onChange={(e) =>
                    setShareMessage(e.target.value.slice(0, 1000))
                  }
                  rows={3}
                ...
                />
                <span className="block mt-1 text-right text-[10px] text-gray-400">

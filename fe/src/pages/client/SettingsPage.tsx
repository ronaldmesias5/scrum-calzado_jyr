...
export default function SettingsPage() {
  ...
  const [appearance, setAppearance] = useState<AppearanceSettings>(() =>
    loadPref('clt_cfg_appearance', { compactView: false, fontSize: 'base' })
  );
  ...
  const [sessionTimeout, setSessionTimeout] = useState(() =>
    loadPref('clt_cfg_sessionTimeout', '60')
  );
  ...
  function handleSave() {
    try {
      localStorage.setItem('clt_cfg_notif', JSON.stringify(notif));
      localStorage.setItem('clt_cfg_lang', JSON.stringify(lang));
      localStorage.setItem('clt_cfg_appearance', JSON.stringify(appearance));
      localStorage.setItem(
        'clt_cfg_sessionTimeout',
        JSON.stringify(sessionTimeout)
      );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 stagger-reveal">
            ...
            <Card title="Preferencias de Notificación">
              <div className="space-y-6">
                <ToggleSetting
                  ...
                  desc="Notificar cuando un pedido tuyo cambie de estado."
                  enabled={notif.orderStatus}
                  onToggle={() =>
                    setNotif((p) => ({ ...p, orderStatus: !p.orderStatus }))
                  }
                />
                <ToggleSetting
                  icon={CheckCircle2}
                  title="Reclamo aprobado"
                  desc="Notificar cuando el jefe apruebe tu reclamo de incidencia."
                  enabled={notif.claimApproved}
                  onToggle={() =>
                    setNotif((p) => ({ ...p, claimApproved: !p.claimApproved }))
                  }
                />
                <ToggleSetting
                  icon={AlertCircle}
                  title="Reclamo rechazado"
                  desc="Notificar cuando el jefe rechace tu reclamo de incidencia."
                  enabled={notif.claimRejected}
                  onToggle={() =>
                    setNotif((p) => ({ ...p, claimRejected: !p.claimRejected }))
                  }
                />
              </div>
            </Card>
            ...
            <Card title="Idioma y Región">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  <label className="text-sm font-bold text-gray-700 dark:text-gray-300 flex items-center gap-2">
                    <Globe className="w-4 h-4 text-blue-500" /> Idioma de la
                    Interfaz
                  </label>
                  <SelectField
                ...
                <div className="space-y-2">
                  ...
                  />
                  <p className="text-xs text-gray-400">
                    Ejemplo: {new Date().toLocaleDateString('es-CO')}
                  </p>
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-bold text-gray-700 dark:text-gray-300">
              ...
              <div className="mt-6 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-2xl border border-blue-100 dark:border-blue-800">
                <p className="text-xs text-blue-700 dark:text-blue-300 font-medium">
                  ...
                  Los cambios de idioma se aplican al hacer clic en{' '}
            ...
            <>
              ...
              <Card title="Sesión">
                <div className="space-y-6">
                  <div className="space-y-2">
                    <label className="text-sm font-bold text-gray-700 dark:text-gray-300 flex items-center gap-2">
                      <Clock className="w-4 h-4 text-blue-500" /> Tiempo de
                      Inactividad para Cerrar Sesión
                    </label>
                    <SelectField
                    ...
                    />
                    <p className="text-xs text-gray-400">
                      La sesión se cerrará automáticamente tras este tiempo sin
                      actividad.
                    </p>
                  </div>
                  ...
                  <div className="pt-4 border-t border-gray-100 dark:border-slate-800">
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1">
                        <h4 className="text-sm font-extrabold text-gray-900 dark:text-white uppercase tracking-tight">
              ...
              <Card title="⚠️ Zona Peligrosa">
                <div className="space-y-4">
                  <div className="rounded-xl border-2 border-red-200 bg-red-50 dark:border-red-900 dark:bg-red-950/30 p-5">
                    <div className="flex items-start gap-4">
                      <div className="w-10 h-10 rounded-xl bg-red-100 dark:bg-red-900/50 flex items-center justify-center text-red-600 dark:text-red-400 flex-shrink-0">
                        <Trash2 className="w-5 h-5" />
                      </div>
                      <div className="flex-1">
                        <h4 className="text-sm font-extrabold text-red-800 dark:text-red-300 uppercase tracking-tight">
                          Eliminar Cuenta
                        </h4>
                        <p className="text-xs text-red-600 dark:text-red-400 font-medium mt-1">
                          Esta acción es <strong>irreversible</strong>. Se
                          eliminará tu cuenta y todos tus datos asociados. Ya no
                          podrás acceder al sistema ni recuperar tu información.
                        </p>
                        <button
                          onClick={async () => {
                            const pw = window.prompt(
                              '⚠️ ELIMINAR CUENTA — Escribe tu contraseña actual para confirmar:'
                            );
                            if (pw === null) return;
                            if (pw === '') {
                              showToast(
                                'Debes escribir tu contraseña para confirmar.',
                                'error'
                              );
                              return;
                            }
                          ...
                          className="mt-3 flex items-center gap-2 px-5 py-2.5 bg-red-600 hover:bg-red-700 text-white font-bold text-sm rounded-xl transition-all active:scale-95"
                        >
                          <Trash2 className="w-4 h-4" />
                          Eliminar Mi Cuenta
                        </button>
                  ...
                  <div className="flex gap-3 flex-wrap">
                    {[
                      ...
                      <button
                        key={opt.val}
                        onClick={() =>
                          setAppearance((p) => ({ ...p, fontSize: opt.val }))
                        }
                ...
                <div className="border-t border-gray-100 dark:border-slate-800 pt-6">
                  ...
                  <div className="p-4 bg-gray-50 dark:bg-slate-800 rounded-2xl space-y-1">
                    <p className="font-extrabold text-gray-900 dark:text-white">
                      Calzado JyR — Sistema de Gestión
                    </p>
                    <p className="text-gray-500 dark:text-gray-400 text-sm">
                      Esta es una línea de ejemplo en tamaño normal.
                    </p>
...
function Toggle({
  ...
  return (
    ...
    >
      <div
        className={`w-4 h-4 bg-white rounded-full transition-transform shadow ${enabled ? 'translate-x-6' : 'translate-x-0'}`}
      />
    </button>
  );
}
      ...
      <div className="flex items-center justify-between">
        <div className="flex gap-2 flex-wrap">
          {checks.map((c) => (
            <span
              key={c.label}
              className={`text-xs font-semibold ${c.ok ? 'text-green-600 dark:text-green-400' : 'text-gray-400'}`}
            >
              {c.ok ? '✓' : '○'} {c.label}
            </span>
          ))}
        ...
        <span
          className={`text-xs font-bold ${score >= 3 ? 'text-green-600' : score >= 2 ? 'text-yellow-600' : 'text-red-500'}`}
        >
          {labels[score - 1] ?? ''}
        </span>
      </div>

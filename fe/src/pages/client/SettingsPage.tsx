...
import {
  ...
  User, Upload, Trash2, Camera, CheckCircle2,
} from 'lucide-react';
...
interface NotifSettings { orderStatus: boolean; claimApproved: boolean; claimRejected: boolean; }
...
export default function SettingsPage() {
  ...
  const [notif, setNotif] = useState<NotifSettings>(() =>
    loadPref('clt_cfg_notif', { orderStatus: true, claimApproved: true, claimRejected: true })
  );
  ...
  function handleSave() {
    try {
      ...
      localStorage.setItem('clt_cfg_sessionTimeout', JSON.stringify(sessionTimeout));
            ...
            <>
              ...
              <Card title="Información del Usuario">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <InfoField label="Razón Social" value={user?.business_name ?? '—'} />
            ...
            <Card title="Preferencias de Notificación">
              <div className="space-y-6">
                <ToggleSetting
                  ...
                  onToggle={() => setNotif(p => ({ ...p, orderStatus: !p.orderStatus }))}
                />
                <ToggleSetting
                  icon={CheckCircle2}
                  ...
                  onToggle={() => setNotif(p => ({ ...p, claimApproved: !p.claimApproved }))}
                />
                <ToggleSetting
                  icon={AlertCircle}
                  ...
                  onToggle={() => setNotif(p => ({ ...p, claimRejected: !p.claimRejected }))}
                />
              </div>
            </Card>
          )}
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-gray-500 dark:text-gray-400 font-medium mt-0.5">
                          Sesión activa como <strong>{user ? `${user.name} ${user.last_name}` : '—'}</strong>
                        </p>
                        <p className="text-xs text-gray-400 mt-1">Tokens almacenados en sessionStorage.</p>
                      </div>
                    </div>
                  </div>
                </div>

...
export function BrandFormModal({ visible, onClose, initialName, initialDescription, onSubmit }: Props) {
      ...
      <View className="flex-1 justify-end bg-black/50">
        <View className="rounded-t-3xl bg-white p-6 dark:bg-slate-900">
          <Text className="mb-4 text-lg font-bold text-gray-900 dark:text-white">
            {initialName ? 'Editar marca' : 'Nueva marca'}
          </Text>
          <Input label="Nombre" value={name} onChangeText={setName} placeholder="Nombre de la marca" />
          <Input
